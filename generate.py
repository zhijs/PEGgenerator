...
atoms = np.zeros((100000,6))
cptatoms = 0
direaction = 0 # 0: 正x方向扩展， 1: 负x方向扩展，2: 正 y 方向扩展
preX = 0 # 上一步的 x 的坐标
preY = 0 # 上一步 y 的坐标
detalx = 0 # x 值的改变方向
endY = 0 # 往 x 方向扩展到最后 Y 的值
monoX = 0 # 单体 x 坐标
monoY = 0 # 单体 y 坐标
monoLen = 7
j = 1
        ...
        if direaction == 0:
            ...
            #   print('x+++', x)
              if (x > Lx and i == 7):
                ...
                preX = x
                atoms[cptatoms] = cptatoms+1, m[1], m[2], x, monoY+m[4], m[5]
        # 往 -x 方向扩展
        elif direaction == 1:
            if i == 1:
              monoX = monoX - v*2
        ...
        # 往 y 正向扩展，
            ...
            if j %2 == 0:
                if detalx > 0:
                    ...
                    direaction = 0
                continue
            atoms[cptatoms] = cptatoms+1, m[1], m[2], monoX+m[3], y, m[5]
        # x 超过盒子宽度， y 方向重新复制
        cptatoms += 1   
    # print('atoms:', atoms)  
# place patch 2
for m in endpatch:
    if direaction == 0:
      atoms[cptatoms] = cptatoms+1, m[1], m[2], m[3]+seg*v+v, m[4]+monoY, m[5]
    elif direaction == 1:
      atoms[cptatoms] = cptatoms+1, m[1], m[2], m[3]-seg*v-v, m[4]+monoY, m[5]
    else:
      atoms[cptatoms] = cptatoms+1, m[1], m[2], m[3]+monoX, m[4]+seg*v+v, m[5]
    cptatoms += 1
